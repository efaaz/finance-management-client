...
const ChartTooltipContent = React.forwardRef((props, ref) => {
  ...
  }, [hideLabel, payload, label, labelFormatter, labelClassName, config, labelKey]);
...
function getPayloadConfig(config, payload, key) {
  const pl = payload && payload.payload ? payload.payload : payload;
  let labelKey = key;
  if (key in payload && typeof payload[key] === "string") labelKey = payload[key];
  else if (pl && key in pl && typeof pl[key] === "string") labelKey = pl[key];
